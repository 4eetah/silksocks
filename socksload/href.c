...
#define HREFREGEX "<a.*?href=[\"'](?<url>http.*?)[\"'].*?>"
...
int myPCRE_href(const char *html, size_t htmlsize, struct URLqueue *Queue)
{
pcre2_code *re;
...
pattern = (PCRE2_SPTR)HREFREGEX;
subject = (PCRE2_SPTR)html;
subject_length = htmlsize;
...
if (namecount == 0) printf("No named substrings\n"); else
  ...
  for (i = 0; i < namecount; i++)
    ...
    URLQput(Queue, subject + ovector[2*n], (int)(ovector[2*n+1] - ovector[2*n]));
    }
  }
